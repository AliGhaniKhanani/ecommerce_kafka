...
async def consume_messages():
    ...
    try:
        async for message in consumer:
            ...
            print(f"Consumer Received message: {message.value.decode('utf-8')} on topic {message.topic}")
            ...
            with next(get_session()) as session:
                ...
                if message["partition_name"] == "update_topic":
                    update_user_in_db(update_user = User(**message["user_data"]), session = session)
                    print("User Info Updated")
                elif message["partition_name"] == "add_topic":
                    add_new_user_in_db(user_data = User(**message["user_data"]), session = session)
...
async def create_user(user: User,session: Annotated[Session, Depends(get_session)],producer: Annotated[AIOKafkaProducer, Depends(get_kafka_producer)])->User:
    db_user = session.query(User).filter(User.email == user.email).one_or_none()
    if db_user:
        print("Email already registered")
        raise HTTPException(status_code=400, detail="Email already registered")
    print("BEFORE JSON FORMAT",user)
    # # Produce message
    message = {
        "partition_name" : "add_topic",
...
@app.post("/Login")
async def login_request(user_data: Annotated[OAuth2PasswordRequestForm, Depends()], session: Annotated[Session, Depends(get_session)]):
    db_user = session.query(User).filter(User.user_name == user_data.username).one_or_none()
    db_password = session.query(User).filter(User.password == user_data.password).one_or_none()
    if not db_user or not db_password:
        print("Username Does Not Exist Please Sign Up")
        raise HTTPException(status_code=400, detail="Username Does Not Exist Please Sign Up")
    if not db_password:
        print("Password Does Not Match")
        raise HTTPException(status_code=400, detail="Username And Password Does Not Match")
    
    generate_token = create_access_token(user_data.username, expire_time= timedelta(minutes=15))
        
    return {"user_data": user_data.username, "Generated Token": generate_token}
